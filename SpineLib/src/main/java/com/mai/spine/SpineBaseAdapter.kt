...
abstract class SpineBaseAdapter : ApplicationAdapter {
    ...
    private val TAG = "SpineBaseAdapter"
...
//    private lateinit var mSkeletonJson: SkeletonBinary
    ...
    private fun initialize() {
        ...
        mSkeletonData = SkeletonBinary(mAtlas).readSkeletonData(mSkeletonFileHandle)

        Log.i(TAG, "onCreate: 获取到的  animations ：${mSkeletonData?.animations}")
        Log.i(TAG, "onCreate: 获取到的  defaultSkin ：${mSkeletonData?.defaultSkin}")
        Log.i(TAG, "onCreate: 获取到的  skins ：${mSkeletonData?.skins}")
        Log.i(TAG, "onCreate: 获取到的  height ：${mSkeletonData?.height}")
        Log.i(TAG, "onCreate: 获取到的  width ：${mSkeletonData?.width}")
        Log.i(TAG, "onCreate: 获取到的  width ：${mSkeletonData?.name}")
...
//        Log.e("TAG", "-----------------------------------------");
        ...
        scaleSkeletonSize(mSkeleton!!, mSkeletonData?.width!!, mSkeletonData?.height!!)
...
//            mSkeleton.setX(mSkeleton.getX()+0.1f);
    ...
    open fun scaleW(width: Float): Float {
//        val scale = context.resources.displayMetrics.density
        val widthNew = Math.round(width / 3 * 3)
        Log.i(
            "ScaleUtil",
            "scaleW  scale --- 3.0 ------------------------ $width - -缩放后-宽- 前 -- $widthNew"
        )
        return widthNew.toFloat()
    }

    open fun scaleH(heigth: Float): Float {
//        val scale = context.resources.displayMetrics.density
        val heigthNew = Math.round(heigth / 3 * 3)
        Log.i(
            "ScaleUtil",
            "scaleWH  scale --- 3.0 ------------------------$heigth -缩放后-高- 前  $heigthNew"
        )
        return heigthNew.toFloat()
    }

    open fun scaleSkeletonSize(theSkeleton: Skeleton, pendantW: Float, pendantH: Float) {
        val scale: Float = getScale(scaleW(pendantW), scaleH(pendantH), pendantW, pendantH)
        theSkeleton.setScale(scale, scale)
        theSkeleton.setPosition((pendantW / 2), 0f)
    }
    open fun getScale(viewW: Float, viewH: Float, jsonW: Float, jsonH: Float): Float {
        return viewH / jsonH
    }
}